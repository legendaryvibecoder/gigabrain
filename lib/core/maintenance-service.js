...
import { renderVaultMirrorMarkdown, syncVaultMirror } from './vault-mirror.js';
import {
...
const runMaintenance = ({
  ...
  const eventCounts = {
    ...
    vault_mirror_copied_files: 0,
    vault_mirror_removed_files: 0,
  ...
  try {
    ...
    const vaultMirrorSummary = syncVaultMirror({
      config,
      dryRun,
    });
    eventCounts.vault_mirror_copied_files = Number(vaultMirrorSummary.copied_files || 0);
    eventCounts.vault_mirror_removed_files = Number(vaultMirrorSummary.removed_files || 0);
    emit({
      timestamp: nowIso,
      component: 'maintenance',
      action: 'vault_mirror',
      reason_codes: [vaultMirrorSummary.enabled === true ? 'complete' : 'disabled'],
      memory_id: `run:${resolvedRunId}`,
      payload: vaultMirrorSummary,
    });
    ...
    const vaultMirrorReportPath = path.join(outputDir, `vault-mirror-${dateKey}.md`);
    fs.writeFileSync(vaultMirrorReportPath, renderVaultMirrorMarkdown({
      timestamp: nowIso,
      runId: resolvedRunId,
      summary: vaultMirrorSummary,
    }), 'utf8');
    ...
    const executionArtifactPath = writeExecutionArtifact({
      ...
      payload: {
        ...
        artifacts: {
          ...
          vault_mirror_report_path: vaultMirrorReportPath,
          vault_root: vaultMirrorSummary.vault_root || '',
    ...
    return {
      ...
      artifacts: {
        ...
        vaultMirrorReportPath,
        vaultRoot: vaultMirrorSummary.vault_root || '',
