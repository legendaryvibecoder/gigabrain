...
// (neutral → 0), range [-0.2, +0.2]. This lets a high-trust source overcome the
// recency boost (max 0.08) of a fresher low-trust belief, while confidence (the
// 0..1 dominant term) still decides when one belief is far better supported.
const beliefHostTrustBonus = (belief = {}) => {
  ...
  return Math.max(-0.2, Math.min(0.2, (w - 0.5) * 0.4));
};
