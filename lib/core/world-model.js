...
// risk). Trust weights are supplied via config.worldModel.hostTrust
// ({ "<host>": 0..1 }) and applied at rebuild time. Empty/absent → no trust
// term, so scoring is unchanged (fully backward compatible).
...
// Map a host's trust in [0,1] to a bounded additive score term. Centered at 0.5
// (neutral → 0), range [-0.1, +0.1] (a 0.2 full swing). The swing is sized to
// beat the recency boost (max 0.08) so a high-trust source can overcome a
// fresher low-trust belief — the drift fix — yet stay smaller than any
// substantial confidence gap, so confidence (the dominant 0..1 term) still
// wins when one belief is clearly better supported. Trust is a tie-breaker,
// not a veto.
