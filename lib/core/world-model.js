...
import { buildEntityMentionScopeFilter, ensurePersonStore } from './person-service.js';
import { ensureProjectionStore, listCurrentMemories } from './projection-store.js';
import { isWorkspaceIdentityAlias } from './workspace-identity.js';
...
const PROJECT_MEMORY_RE = /\b(?:project|startup|company|organization|organisation|investment|investor|valuation|interview|rollout|plugin|integration|bot|feature|api|provider)\b/i;
...
// Deployment-specific claim slots are NOT hardcoded — that overfits the engine
// to one user. Instead they are supplied via config.worldModel.customSlotRules
// so any user/agent can define durable slots for their own projects/people.
// Each rule: { pattern, flags?, slot, topic, subtopic, value?, operation? }.
// Defaults to empty, so a fresh install relies purely on the generic detectors.
let CUSTOM_SLOT_RULES = [];
const configureWorldModelRules = (config = {}) => {
  const raw = Array.isArray(config?.worldModel?.customSlotRules)
    ? config.worldModel.customSlotRules
    : [];
  const compiled = [];
  for (const rule of raw) {
    ...
    let re;
    try { re = new RegExp(pattern, typeof rule.flags === 'string' ? rule.flags : 'i'); } catch { continue; }
    compiled.push({
      re,
      slot,
      topic: String(rule.topic || '').trim(),
      subtopic: String(rule.subtopic || '').trim(),
      value: rule.value === undefined || rule.value === null ? null : String(rule.value),
      operation: String(rule.operation || 'update').trim() || 'update',
    });
  }
  CUSTOM_SLOT_RULES = compiled;
  return CUSTOM_SLOT_RULES;
};
const matchCustomSlotRule = (content = '') => {
  const text = String(content || '');
  if (!text) return null;
  for (const rule of CUSTOM_SLOT_RULES) {
    if (rule.re.test(text)) {
      return {
        slot: rule.slot,
        normalizedValue: rule.value || summarizeContent(text, 120),
        topic: rule.topic,
...
const normalizeRequestedScope = (value) => String(value || '').trim();
...
const isScopeVisibleForRequested = (rowScope = '', requestedScope = '') => {
  const requested = normalizeRequestedScope(requestedScope);
  if (!requested) return true;
  const actual = normalizeScope(rowScope);
  if (requested === 'shared') return actual === 'shared';
  if (!requested.startsWith('project:') && !requested.startsWith('profile:')) {
    return actual === requested || actual === 'shared' || (requested === 'main' && actual === 'profile:main');
  }
...
const normalizeScopeList = (value = []) => (
  Array.isArray(value)
    ? Array.from(new Set(value.map((item) => normalizeScope(item)).filter(Boolean)))
    : []
);

const isEntityVisibleInScope = (payload = {}, scope = '') => {
  const requested = normalizeRequestedScope(scope);
  if (!requested) return true;
  const scopes = normalizeScopeList(payload?.scopes);
  if (scopes.length === 0) return false;
  return scopes.some((rowScope) => isScopeVisibleForRequested(rowScope, requested));
};

const shouldExposeEntitySyntheses = (payload = {}, scope = '') => {
  const requested = normalizeRequestedScope(scope);
  if (!requested) return true;
  const scopes = normalizeScopeList(payload?.scopes);
  if (scopes.length === 0) return false;
  if (scopes.length > 1) return false;
  return isScopeVisibleForRequested(scopes[0], requested);
};
...
const scoreCuratedSurfaceEntity = (entity = {}, beliefs = [], episodes = [], config = {}) => {
  ...
  if (entity.kind === 'person') {
    ...
    const declaredWorkspaceIdentity = workspaceRoot
      ? isWorkspaceIdentityAlias(entity.normalized_name || entity.display_name || '', workspaceRoot)
      : false;
    const hasAgentIdentityEvidence = currentBeliefs.some((belief) => String(belief.type || '').trim().toUpperCase() === 'AGENT_IDENTITY');
    const hasRelationshipEvidence = currentBeliefs.some((belief) => String(belief.type || '').trim().toLowerCase() === 'relationship');
    if (!(declaredWorkspaceIdentity || hasAgentIdentityEvidence || hasRelationshipEvidence)) return -1;
...
// risk). Trust weights come from config.worldModel.hostTrust ({ host: 0..1 }),
// applied at rebuild. Empty/absent → no term, scoring unchanged.
let BELIEF_HOST_TRUST = null;
const configureBeliefTrust = (config = {}) => {
  const raw = config && config.worldModel && typeof config.worldModel.hostTrust === 'object'
    ? config.worldModel.hostTrust
    : null;
  if (!raw) { BELIEF_HOST_TRUST = null; return null; }
  const map = {};
  ...
  BELIEF_HOST_TRUST = Object.keys(map).length > 0 ? map : null;
  return BELIEF_HOST_TRUST;
};
...
const beliefHostTrustBonus = (belief = {}) => {
  if (!BELIEF_HOST_TRUST) return 0;
  const host = String(belief.source_host || belief.source_agent || '').trim().toLowerCase();
  if (!host) return 0;
  const w = BELIEF_HOST_TRUST[host];
...
const beliefPriorityScore = (belief = {}) => {
  ...
  score += beliefHostTrustBonus(belief);
  return score;
};
...
const normalizeClaimSlotFromBelief = (belief = {}) => {
  ...
  if (!content) return null;
  const customSlot = matchCustomSlotRule(content);
  ...
  if (/\b(?:birthday|geburtstag)\b/i.test(content)) {
    return {
...
const findSurfacePreferredGivenName = (entityRow = {}, relatedMemoryRows = [], availableKeys = new Set(), config = {}) => {
  if (String(entityRow.kind || '') !== 'person') return '';
  const workspaceRoot = String(config?.runtime?.paths?.workspaceRoot || '').trim();
  if (workspaceRoot && isWorkspaceIdentityAlias(entityRow.normalized_name || entityRow.display_name || '', workspaceRoot)) return '';
...
const buildVisibleEntityAliasesForScope = (db, entityKey = '', scope = '') => {
  const requested = normalizeRequestedScope(scope);
  const normalizedEntityKey = normalizeContent(entityKey);
  if (!requested || !normalizedEntityKey || !hasTable(db, 'memory_entity_mentions')) return [];
  ensurePersonStore(db);
  const scopeFilter = buildEntityMentionScopeFilter(requested, 'scope');
  const rows = db.prepare(`
    SELECT entity_key, entity_display
    FROM memory_entity_mentions
    WHERE lower(trim(entity_key)) = ?
      AND ${scopeFilter.sql}
    ORDER BY confidence DESC, entity_display ASC
    LIMIT 50
  `).all(normalizedEntityKey, ...scopeFilter.params);
  return Array.from(new Set(rows.flatMap((row) => [
    String(row?.entity_display || '').trim(),
    String(row?.entity_key || '').trim(),
  ]).filter(Boolean)));
};

const buildVisibleEntityKeysForScope = (db, scope = '') => {
  const requested = normalizeRequestedScope(scope);
  if (!requested || !hasTable(db, 'memory_entity_mentions')) return null;
  ensurePersonStore(db);
  const scopeFilter = buildEntityMentionScopeFilter(requested, 'scope');
  const rows = db.prepare(`
    SELECT entity_key, entity_display
    ...
    GROUP BY entity_key, entity_display
    ORDER BY COUNT(*) DESC
    LIMIT 1000
  `).all(...scopeFilter.params);
  return new Set(rows.flatMap((row) => [
    normalizeContent(row?.entity_key || ''),
    normalizeContent(row?.entity_display || ''),
  ]).filter(Boolean));
};

const buildSourceScopeLookup = (db, sourceIds = []) => {
  ...
  for (const sourceId of ids) {
    const row = query.get(sourceId);
    if (!row) continue;
    lookup.set(sourceId, normalizeScope(row.scope));
  }
  return lookup;
};
...
const inferEntityKind = (entityKey, memoryRows = [], mentionRows = [], config = {}) => {
  ...
  const normalizedTypes = memoryRows.map((row) => String(row.type || '').trim().toUpperCase());
  const workspaceRoot = String(config?.runtime?.paths?.workspaceRoot || '').trim();
  ...
  const hasAgentIdentityEvidence = normalizedTypes.includes('AGENT_IDENTITY');
  const hasPersonalScopeEvidence = memoryRows.some((row) => {
    const scope = normalizeScope(row.scope);
    return scope === 'main' || scope.startsWith('profile:');
  });
  const hasPersonalMemoryType = normalizedTypes.some((type) => ['AGENT_IDENTITY', 'USER_FACT', 'PREFERENCE', 'DECISION'].includes(type));
  const hasAliasProfileCue = memoryRows.some((row) => {
    const content = String(row.content || '');
    return hasAliasCueWithinWindow(content, entityKey, new Set(['founder', 'ceo', 'role', 'job', 'title', 'based', 'lives', 'prefers', 'likes', 'london', 'vienna', 'wien']), 6)
      || hasPlacePatternForAlias(content, entityKey);
  });
  ...
  const hasProjectCue = memoryRows.some((row) => hasAliasCueWithinWindow(
    row.content,
    entityKey,
    new Set([...PROJECT_CUE_TOKENS, 'model', 'tool', 'provider', 'weights', 'api', 'openrouter', 'moonshot', 'oauth', 'dashboard', 'integration']),
    5,
  ));
  const hasStrongProjectEvidence = memoryRows.some((row) => {
    const content = String(row.content || '');
    return PROJECT_IDENTITY_RE.test(content) || PROJECT_MEMORY_RE.test(content) || SURFACE_PROJECT_PREFERRED_RE.test(content);
  });
  const hasPlaceCue = memoryRows.some((row) => hasPlacePatternForAlias(row.content, entityKey));
  const isDeclaredWorkspaceIdentity = workspaceRoot ? isWorkspaceIdentityAlias(entityKey, workspaceRoot) : false;
  if (isLikelyPersonAlias(entityKey) && isEntityKindEnabled(config, 'person')) {
    if (hasAgentIdentityEvidence) return 'person';
    if (hasRelationshipMention || hasPublicProfileMention) return 'person';
    if (RELATIONSHIP_RE.test(text)) return 'person';
    if (hasAliasProfileCue) return 'person';
    if (isDeclaredWorkspaceIdentity && hasPersonalScopeEvidence && hasPersonalMemoryType && !hasStrongProjectEvidence) return 'person';
  }
  if (hasOrganizationCue && isEntityKindEnabled(config, 'organization')) return 'organization';
  if ((hasProjectCue || hasStrongProjectEvidence) && isEntityKindEnabled(config, 'project')) return 'project';
...
const evaluateEntityCandidate = ({
  ...
  if (kind === 'person') {
    ...
    const workspaceRoot = String(config?.runtime?.paths?.workspaceRoot || '').trim();
    const declaredWorkspaceIdentity = workspaceRoot
      ? filteredAliases.some((alias) => isWorkspaceIdentityAlias(alias, workspaceRoot))
      : false;
    const hasAgentIdentityEvidence = relatedMemoryRows.some((row) => String(row.type || '').trim().toUpperCase() === 'AGENT_IDENTITY');
    const hasAliasProfileCue = relatedMemoryRows.some((row) => filteredAliases.some((alias) => (
      hasAliasCueWithinWindow(String(row.content || ''), alias, new Set(['founder', 'ceo', 'role', 'job', 'title', 'based', 'lives', 'prefers', 'likes', 'london', 'vienna', 'wien']), 6)
      || hasPlacePatternForAlias(String(row.content || ''), alias)
    )));
    ...
    if (!declaredWorkspaceIdentity && !hasAgentIdentityEvidence && !strongMention && !hasAliasProfileCue) {
      return { accepted: false, kind, aliases: filteredAliases, reason: 'weak_person_identity_signal' };
    }
  }
...
const rebuildWorldModel = ({
  ...
  ensureWorldModelStore(db);
  configureWorldModelRules(config);
  configureBeliefTrust(config);
  ensureProjectionStore(db);
  ensurePersonStore(db);
  ...
  for (const entityRow of entityRows) {
    ...
    const entityKey = normalizeContent(entityRow.normalized_name || entityRow.display_name || '');
    const relatedMemoryRows = relatedRowsByEntityKey.get(entityKey) || [];
    const entityMentions = mentionsByKey.get(entityKey) || [];
    const declaredWorkspaceIdentity = workspaceRoot ? isWorkspaceIdentityAlias(entityKey, workspaceRoot) : false;
    const hasStrongSurfaceIdentitySignal = declaredWorkspaceIdentity
      || relatedMemoryRows.some((row) => String(row.type || '').trim().toUpperCase() === 'AGENT_IDENTITY')
      || entityMentions.some((row) => ['relationship', 'public_profile'].includes(String(row.role || '').toLowerCase()));
    if (!hasStrongSurfaceIdentitySignal) {
      entityRow.payload = {
        ...entityRow.payload,
        surface_visible: false,
        surface_suppressed_reason: 'weak_person_identity_signal',
      };
      continue;
    }
    const preferredGivenName = findSurfacePreferredGivenName(entityRow, relatedMemoryRows, availableEntityKeys, config);
  ...
  for (const entity of entityRows) {
    ...
    const surfacePriority = scoreCuratedSurfaceEntity(entity, entityBeliefs, entityEpisodes, config);
    entity.payload = {
      ...entity.payload,
...
const findEntityMatches = (db, query, options = {}) => {
  ...
  const requestedScope = normalizeRequestedScope(options.scope);
  const visibleEntityKeys = buildVisibleEntityKeysForScope(db, requestedScope);
  ...
  const aliases = db.prepare(`
    SELECT a.entity_id, a.alias, a.normalized_alias, a.confidence, e.kind, e.display_name, e.normalized_name, e.status, e.updated_at, e.confidence AS entity_confidence, e.payload
  ...
  const scored = aliases.map((row) => {
    ...
    if (requestedScope && !isEntityVisibleInScope(payload, requestedScope)) return null;
    if (visibleEntityKeys && visibleEntityKeys.size > 0) {
      const normalizedName = normalizeContent(row.normalized_name || row.display_name || '');
      if (!visibleEntityKeys.has(normalizedAlias) && !visibleEntityKeys.has(normalizedName)) {
        return null;
      }
    }
    let lexicalScore = 0;
...
const getEntityDetail = (db, entityId, options = {}) => {
  ensureWorldModelStore(db);
  const requestedScope = normalizeRequestedScope(options.scope);
  ...
  if (requestedScope && !isEntityVisibleInScope(entity.payload, requestedScope)) return null;
  ...
  const sourceScopeLookup = buildSourceScopeLookup(db, [
    ...beliefs.map((belief) => belief.source_memory_id),
    ...episodes.flatMap((episode) => episode.source_memory_ids),
    ...openLoops.flatMap((loop) => loop.source_memory_ids),
  ]);
  const scopedBeliefs = requestedScope
    ? beliefs.filter((belief) => {
      const sourceId = String(belief.source_memory_id || '').trim();
      if (!sourceId || !sourceScopeLookup.has(sourceId)) return false;
      return isScopeVisibleForRequested(sourceScopeLookup.get(sourceId), requestedScope);
    })
    : beliefs;
  const scopedEpisodes = requestedScope
    ? episodes.filter((episode) => episode.source_memory_ids.some((sourceId) => {
      const normalizedSourceId = String(sourceId || '').trim();
      if (!normalizedSourceId || !sourceScopeLookup.has(normalizedSourceId)) return false;
      return isScopeVisibleForRequested(sourceScopeLookup.get(normalizedSourceId), requestedScope);
    }))
    : episodes;
  const scopedOpenLoops = requestedScope
    ? openLoops.filter((loop) => loop.source_memory_ids.some((sourceId) => {
      const normalizedSourceId = String(sourceId || '').trim();
      if (!normalizedSourceId || !sourceScopeLookup.has(normalizedSourceId)) return false;
      return isScopeVisibleForRequested(sourceScopeLookup.get(normalizedSourceId), requestedScope);
    }))
    : openLoops;
  const scopedAliases = requestedScope
    ? buildVisibleEntityAliasesForScope(db, entity.normalized_name || entity.display_name, requestedScope)
    : listEntityAliases(db, entity.entity_id).map((aliasRow) => aliasRow.alias);
  return {
    ...entity,
    aliases: scopedAliases.length > 0 ? scopedAliases : [entity.display_name].filter(Boolean),
    beliefs: scopedBeliefs,
    episodes: scopedEpisodes,
    open_loops: scopedOpenLoops,
    syntheses: shouldExposeEntitySyntheses(entity.payload, requestedScope)
      ? listSyntheses(db, { subjectType: 'entity', subjectId: entity.entity_id, limit: 20 })
      : [],
  };
};
