import { detectTemporalWindow, estimateTokens, recallForQuery } from './recall-service.js';
import {
...
const orchestrateRecall = ({
  ...
  const entityMatches = findEntityMatches(db, query, {
    limit: 5,
  ...
  if (!topEntity && ['entity_brief', 'relationship_brief', 'timeline_brief', 'contradiction_check'].includes(strategy)) {
  ...
  const recall = deepLookupAllowed
    ? recallForQuery({
      db,
      config,
      query,
  ...
  return {
    ...
    deepLookupReason,
    deepLookupReasons,
    rankingMode: String(recall.rankingMode || 'broad'),
    usedWorldModel,
    temporalWindow,
    entityMatches,
    ...
    explain: {
      ...
      profile,
      reason: intent.reason,
      ranking_mode: String(recall.rankingMode || 'broad'),
