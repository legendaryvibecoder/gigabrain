...
import { rewriteSelfReferenceQuery } from './workspace-identity.js';
import {
...
const ENTITY_PREFERENCE_QUERY_RE = /\b(?:prefer(?:s)?|like(?:s)?|love(?:s)?|hate(?:s)?|dislike(?:s)?|bevorzug(?:t|en)?|mag|liebt|hasst)\b/i;
const QUESTION_LEAD_RE = /^\s*(?:what|which|does|do|was|welche|welcher|welches)\b/i;
...
const promoteQuickContextEntityBrief = ({
  query = '',
  strategy = 'quick_context',
  topEntity = null,
} = {}) => {
  const baseStrategy = String(strategy || '').trim() || 'quick_context';
  if (baseStrategy !== 'quick_context') return { strategy: baseStrategy, reason: '' };
  if (!topEntity?.entity_id) return { strategy: baseStrategy, reason: '' };
  const kind = String(topEntity?.kind || '').trim().toLowerCase();
  if (!['person', 'project', 'organization', 'place'].includes(kind)) {
    return { strategy: baseStrategy, reason: '' };
  }
  const text = String(query || '').trim();
  if (!text || !QUESTION_LEAD_RE.test(text) || !ENTITY_PREFERENCE_QUERY_RE.test(text)) {
    return { strategy: baseStrategy, reason: '' };
  }
  return { strategy: 'entity_brief', reason: 'entity_preference' };
};
...
const orchestrateRecall = ({
  ...
  const selfReference = rewriteSelfReferenceQuery(String(query || '').trim(), {
    workspaceRoot: config?.runtime?.paths?.workspaceRoot || '',
  });
  const sanitizedQuery = sanitizeRecallQuery(selfReference.query || query);
  const effectiveQuery = sanitizedQuery || String(selfReference.query || query || '').trim();
  ...
  const promotedStrategy = promoteQuickContextEntityBrief({
    query: effectiveQuery,
    strategy: intent.strategy,
    topEntity,
  });
  const initialStrategy = promotedStrategy.strategy;
  const strategyReason = promotedStrategy.reason || intent.reason;
  ...
  const shouldRefreshRecall = deepLookupAllowed || strategy !== initialStrategy;
