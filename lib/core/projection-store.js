...
const scopeWhereForRequested = (scope) => {
  ...
  return { sql: 'memory_current.scope = ?', params: [requested] };
};

const rowVisibleForRequestedScope = (rowScope = '', requestedScope = '') => {
  const requested = String(requestedScope || '').trim();
  if (!requested) return true;
  const actual = String(rowScope || '').trim();
  if (requested === 'shared') return actual === 'shared';
  if (requested.startsWith('project:') || requested.startsWith('profile:')) return actual === requested || actual === 'shared';
  ...
  for (const [memoryId, bonus] of ftsBoost) {
    ...
    try {
      ...
      if (ftsRow) {
        ...
        if (scope && !rowVisibleForRequestedScope(ftsRow.scope, scope)) continue;
