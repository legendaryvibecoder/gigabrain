...
const listCurrentMemories = (db, options = {}) => {
  ...
  if (scope) {
    // Include 'shared' scope — cross-project by design (matches person-service.js pattern)
    where.push("(memory_current.scope = ? OR memory_current.scope = 'shared')");
    params.push(scope);
  }
  ...
  for (const [memoryId, bonus] of ftsBoost) {
    ...
    try {
      ...
      if (ftsRow) {
        ...
        if (scope && String(ftsRow.scope || '').trim() !== scope && String(ftsRow.scope || '').trim() !== 'shared') continue;
