...
const searchCurrentMemories = (db, options = {}) => {
  ...
  where.push(`(${tokens.map(() => '(memory_current.content LIKE ? OR memory_current.normalized LIKE ?)').join(' OR ')})`);
  for (const token of tokens) {
    const like = `%${token}%`;
  ...
  for (const [memoryId, bonus] of ftsBoost) {
    ...
    try {
      const ftsRow = db.prepare('SELECT memory_current.* FROM memory_current WHERE memory_id = ?').get(memoryId);
      if (ftsRow) {
        scored.push({
          ...ftsRow,
          score_lexical: 0,
