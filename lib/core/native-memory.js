...
const writeNativeMemoryEntry = ({
  ...
  // MEMORY.md lives at the workspace root (the parent of memoryRoot), while daily
  // notes live under memoryRoot. The #48 path-traversal guard must be given the
  // directory that actually contains the target file, or a legit durable write to
  // MEMORY.md is wrongly rejected (relative path starts with '..').
  const allowedRoot = durable ? path.dirname(memoryMdPath) : memoryRoot;
