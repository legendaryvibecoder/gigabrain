...
const validateQueuePath = (queuePath, allowedRoot = '') => {
  if (!queuePath || typeof queuePath !== 'string') return false;
  if (queuePath.length > MAX_PATH_LENGTH) return false;
  // Block path traversal attempts
  if (queuePath.includes('..')) return false;
  if (queuePath.includes('\0')) return false;

  // If allowedRoot is provided, validate the path is within it
  if (allowedRoot) {
...
const ensureQueueDir = (queuePath, allowedRoot = '') => {
  if (!validateQueuePath(queuePath, allowedRoot)) {
    throw new Error('Invalid queue path: path traversal detected');
  }
...
const acquireQueueLock = (queuePath, options = {}) => {
  ...
  ensureQueueDir(queuePath, allowedRoot);
...
// Secure deserialization: strip ONLY prototype-pollution keys, at every depth,
// and keep every legitimate field. The previous implementation also applied a
// fixed property whitelist, which silently deleted valid queue fields the
// producers actually write (e.g. winner_memory_id / loser_memory_id /
// auto_resolved from the dedupe path, plausibility_flags / matched_pattern from
// capture). That corrupted the review/audit trail on the first retention rewrite
// and made integration-audit-maintenance flaky. A whitelist is the wrong tool
// here — the only security goal is blocking __proto__/constructor/prototype.
const FORBIDDEN_KEYS = new Set(['__proto__', 'constructor', 'prototype']);

const sanitizeRow = (value) => {
  if (Array.isArray(value)) {
    return value.map((item) => (item && typeof item === 'object' ? sanitizeRow(item) : item));
  }
  ...
  for (const key of Object.keys(value)) {
    ...
    const v = value[key];
    out[key] = (v && typeof v === 'object') ? sanitizeRow(v) : v;
  }
  return out;
};
  ...
  for (const line of String(raw || '').split('\n')) {
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object') {
        // Sanitize to prevent prototype pollution and limit property exposure
        const row = sanitizeRow(parsed);
...
const applyQueueRetentionUnlocked = (queuePath, retentionConfig = {}, options = {}) => {
  const allowedRoot = options?.allowedRoot || '';
  // Validate queuePath to prevent path traversal
  if (!validateQueuePath(queuePath, allowedRoot)) {
    throw new Error('Invalid queue path: path traversal detected');
  }
  ...
  if (!dryRun && nextRaw !== raw) {
    ensureQueueDir(queuePath, allowedRoot);
    // Validate tempPath doesn't escape allowed root
    const tempPath = `${queuePath}.tmp-${process.pid}-${Date.now()}`;
    if (!validateQueuePath(tempPath, allowedRoot)) {
      throw new Error('Invalid temp path: path traversal detected');
    }
...
const appendQueueRow = (queuePath, row, options = {}) => {
  ...
  const allowedRoot = options?.allowedRoot || '';
  // Validate queuePath before any operations
  if (!validateQueuePath(queuePath, allowedRoot)) {
    throw new Error('Invalid queue path: path traversal detected');
  }
